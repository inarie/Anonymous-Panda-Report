...
When the Leap Motion Controller has a good, high-contrast view of an object's silhouette, it uses infrared light and optical sensors to perform the detection and tracking work. These sensors have a $150^\circ$ field of view and their effective range extends from approximately 3 to 60 centimetres (see Figure \ref{fig:HMDistance}). Last but not least, the Leap Motion software combines sensor data with an internal model of the human hand to handle complex tracking conditions \cite{ULTP}.
...
% \paragraph{Integration of the Ultraleap Plugin}
% The official Ultraleap Tracking Plugin for Unreal Engine 4 utilizes pre-made blueprints to add hand tracking to a scene and play it in the editor. However, depending on the project, it may take a more customized approach using C\texttt{+}\texttt{+} or blueprints \cite{ULTGIT}.

% To use the Ultraleap Plugin, which is intended to be a simple tool for integrating Ultraleap cameras into Unreal projects, the following prerequisites must be met:
% \begin{enumerate}[leftmargin=3\parindent]
%     \item Ultraleap Tracking Service 5.3.1 was installed;
%     \item Ultraleap compatible device (Leap Motion);
%     \item Unreal 4.27.1.
% \end{enumerate}

% Following that, we must ensure the following steps are performed in order for the Plugin to work properly in the project:
% \begin{enumerate}[leftmargin=3\parindent]
%     \item Download the version 4.2.0 of the Unreal Plugin (should be a .zip file);
%     \item Create a Plugins folder in the project root folder;
%     \item Drag the unzipped UltraleapTracking Plugin into the project's Plugins folder;
%     \item Rename the Unreal Plugin folder to "UltraleapTracking";
%     \item Enable the Plugin via Unreal Editor.
% \end{enumerate}

% To test if the Plugin was correctly installed and enabled, there should be a folder named Examples inside the UltraleapTracking Content (check the view options to see if the "Show Plugin Content" option is enabled) with pre-made assets that can ensure that the plugin works (see Figure \ref{fig:plugin}).

% \begin{figure}[!htb]
% \includegraphics[width=\textwidth]{figures/pluginWorking.jpg}
% \centering
% \caption{A simple example of the plugin in action}
% \label{fig:plugin}
% \end{figure}
